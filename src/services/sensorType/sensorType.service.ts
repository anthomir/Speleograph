...
export class SensorTypeService implements OnInit {
  ...
  async find(req : Req, res: Res, filter?: any) {
    try{
        return res.status(200).json({success: true, data: filter ? await this.SensorType.find(JSON.parse(filter)) : await this.SensorType.find() })
    } catch (err){
  ...
  async post(req : Req, res: Res, body: any){
    try{
      ...
      return res.status(201).json({success: true, data: await this.SensorType.create({name: body.name, properties: body.properties, type: body.type, manufacturer: body.manufacturer, isDefault: false, createdBy: user.id}) })
    } catch (err){
        return res.status(500).json({success: false, err: err })
    }
  }
  ...
  async delete(req : Req, res: Res, filter: any){
    try{
      ...
      return res.status(200).json({success: true, data: await this.SensorType.deleteOne(JSON.parse(filter))  })
    } catch (err){
  ...
  async $onInit() {
    try{
        ...
        await this.SensorType.create({name: SensorTypeEnum.ReefNet, properties: reefArray, isDefault: true})
        await this.SensorType.create({name: SensorTypeEnum.CTDSensor, properties: ctdArray, isDefault: true})
        await this.SensorType.create({name: SensorTypeEnum.PluvioMeter, properties: pluvioArray, isDefault: true})
