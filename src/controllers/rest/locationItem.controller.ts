...
export class LocationItemController {
    ...
    async findById(@Res() res: Res, @PathParams('id') id: string) {
        try {
            ...
            const result = await this.locationItemService.findById({ _id: id, isDeleted: false });
    ...
    async find(
        ...
        try {
            ...
            const result = await this.locationItemService.find(query, skip, take, sortBy);
            if (result.status === 200) {
    ...
    async post(@Context('user') user: User, @Res() res: Res, @BodyParams() body?: any) {
        ...
        try {
            const result = await this.locationItemService.post(body);
    ...
    async update(@Res() res: Res, @PathParams('id') id: string, @BodyParams() body: UpdateSensorTypeDto) {
        try {
            ...
            const result = await this.locationItemService.updateSensorType(id, body);
    ...
    async delete(@Context('user') user: User, @Res() res: Res, @PathParams('id') id: string, @QueryParams('force') force: boolean) {
        try {
            ...
            const sensor = await this.locationItemService.findById({ _id: id, isDeleted: false });
            if (!sensor.data) {
            ...
            if (force == true && user.role == Role.Admin) {
                result = await this.locationItemService.forceDeleteById(id, user);
            } else {
                result = await this.locationItemService.deleteById(id, user);
            }
    ...
    async restore(@Context('user') user: User, @Res() res: Res, @BodyParams('ids') ids: string[]) {
        ...
        try {
            const result = await this.locationItemService.restore({ _id: { $in: ids }, isDeleted: true });
