...
export class CaveController {
	...
	async coordinatesByGeolocation(
		...
		let objRes = await this.caveService.geolocationCoordinateSearch(
			sw_lat,
			sw_lng,
			ne_lat,
			ne_lng
		...
		if (objRes.response == null && objRes.err == null) {
			return res
				.status(404)
				.json({ success: false, err: 'No coordinates found' });
		}
	...
	async CavesByGeolocation(
		...
		let objRes = await this.caveService.geolocationDataSearch(
			sw_lat,
			sw_lng,
			ne_lat,
			ne_lng
