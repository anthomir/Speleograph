...
export class SensorTypeController {
    ...
    async findById(@Res() res: Res, @PathParams('id') id: string) {
        try {
            ...
            const result = await this.sensorTypeService.findById(id);
    ...
    async find(
        ...
        try {
            const result = await this.sensorTypeService.find(filter, skip, take, sortBy);
            if (result.status === 200) {
    ...
    async delete(@Res() res: Res, @PathParams('id') id: string) {
        try {
            const result = await this.sensorTypeService.deleteById(id);
            if (result.status === 404) {
                return res.status(404).json({ sucess: false, err: result.message });
            } else if (result.status === 200) {
